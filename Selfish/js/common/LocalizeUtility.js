class LocalizeUtility {
    ...
    static outputTextdata(){
        ...
        textTemp['materialTitle'] = $dataText['materialTitle'];
        textTemp['materialText'] = $dataText['materialText'];
        
        var _quizData = new Game_Quiz();
        textTemp['quizQuestion'] = $dataText['quizQuestion'];
        textTemp['quizChoices'] = $dataText['quizChoices'];

        textTemp['tipsText'] = $dataText['tipsText'];
        textTemp['tipsHelpText'] = $dataText['tipsHelpText'];
        ...
        this.tips2csv($gameTips._data);
        this.quiz2csv();
        this.help2csv();
        //this.tutorial2csv($gameTutorial._data);
        this.material2csv($gameMaterial._data);
    ...
    static material2csv(json) {
        var body = json.map(function(d){
            return Object.keys(d).map(function(key) {
                ...
                if (key == "text" || key == "title" || key == "item")
                return data;
            }).join(',');
        }).join("\n");
        this.outputcsv('csv/jp/text/' + 'materialText.csv' ,body);
    }
