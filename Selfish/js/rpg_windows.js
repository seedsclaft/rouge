...
Window_Selectable.prototype.drawMpCost = function(mpCost,x,y,width) {
    if (mpCost < 0){
        return;
    }
...
Window_Base.prototype.drawSkillExp = function(skillData,x,y,width) {
    if ($dataSkills[ skillData.skillId ].maxLevel == 0 || $dataSkills[ skillData.skillId ].maxLevel == 1){
        return;
    }
    const lv = skillData.level;
    if (lv == 0){
        return;
    }
    const value = skillData.expRateTotal;
    const value2 = skillData.expRate;
    const color1 = $gameColor.getColor('skillexp1');
    const color2 = $gameColor.getColor('skillexp1');
    const color1a = $gameColor.getColor('skillexp2');
    const color2b = $gameColor.getColor('skillexp2');
    if (lv >= $dataSkills[ skillData.skillId ].maxLevel){
        this.drawGauge(x, y, width, 1, color1a, color2b,true);
    } else
    if (!isNaN(value) || !isNaN(value2)){
        if (value >= value2){
            this.drawGauge(x, y, width, value, color1, color2,true);
            this.drawGauge(x, y, width, value2, color1a, color2b,false);
        } else{
            this.drawGauge(x, y, width, value, color1, color2,true);
        }
...
Window_Base.prototype.drawSkillLevel = function(skillData,max, x, y) {
    if (max == 0 || max == 1){
        return;
    }
    const lv = skillData.level;
    ...
    const lessLv = skillData.lessLevel;
    this.contents.fontSize = 21;
    if (lessLv == $dataSkills[ skillData.skillId ].maxLevel){
        this.drawText(TextManager.getText(10600), x - 4, y + 2, 64, 'center');
        return;
    }
    this.drawText(max, x + 36, y + 3, 24, 'center');

    this.contents.fontSize = 16;
    this.drawText("/", x + 21, y + 1 , 80, 'left'); 
    ...
    this.drawText(lv, levelX, y + 2, 40, 'center');
    this.resetFontSettings();
}
