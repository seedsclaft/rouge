...
EventManager.setupEventBycsv = function(filename,csvPath) {
    if (this._testBusy){
        return;
    }
    this._testBusy = true;
    ...
    let csv = fs.readFileSync(csvPath);
    var iconvlite = require('iconv-lite');
    csv = iconvlite.decode(Buffer.from(csv), "utf-8");
    let dataArray = LocalizeUtility.csvToJson(csv);

    let baseData = fs.readFileSync(base + "\\" + filename.replace(".csv",".json"));
    ...
    baseData.list.forEach((l,index) => {
        ...
        if ( listData.code == 401 ){
            ...
            if (localizeData && localizeData[3] && localizeData[3].length > 1){
                var extraCode = {code:401,indent:0,parameters:[localizeData[3]]};
        ...
        if (listData.code == 402){
            ...
            var localizeData = _.find(dataArray,(data) => data[2] == basetext);
    ...
    baseData.list = _.filter(baseData.list,(l) => {return l.code != 111;});
    baseData.list = _.filter(baseData.list,(l) => {return l.code != 119;});
    baseData.list = _.filter(baseData.list,(l) => {return l.code != 201;});
    this.setupTest(baseData.list);
