...
PopupLevelUpManager.setBeforeMembers = function(members) {
    ...
    members.forEach(actor => {
        let data = {exp:actor.currentExp(),actorId:actor.actorId(),level:actor.level,awakeInfo:[],exp:actor.currentExp()}
...
PopupLevelUpManager.openWindow = function(result) {
    ...
    if (result.level){
        ...
        // レベルアップボイス
        SoundManager.playBattleVoice(actor.actorId(),BattleVoiceType.Levelup);
    } else if (result.skill){
        // スキル習得ボイス
        SoundManager.playBattleVoice(actor.actorId(),BattleVoiceType.Awaking);
    }
